...
class File:
    ...
    def __init__(
        ...
        filename: Optional[str] = None  # <-- added latter
    ):
        self.content = content
        self.acls = acls or {}
        self.url = url
        self._filename = filename  # <-- use to get filename
    ...
    def file_extension(self) -> str:
        try:
            if self._filename:
                return os.path.splitext(self._filename or "")[1]
            else:
                return os.path.splitext(self.content.name)[1]
        except Exception as e:
            logger.error(f"Error to get extension - {e}")
            raise e
