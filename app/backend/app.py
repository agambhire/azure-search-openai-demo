...
from config import (
    ...
    CONFIG_CHAT_WITHOUT_AI_SEARCH_APPROACH,
...
async def setup_clients():
    ...
    # ChatReadApproach is used by /chat for multi-turn conversation
    current_app.config[CONFIG_CHAT_WITHOUT_AI_SEARCH_APPROACH] = ChatReadApproach(
        openai_client=openai_client,
        auth_helper=auth_helper,
        chatgpt_model=OPENAI_CHATGPT_MODEL,
        chatgpt_deployment=AZURE_OPENAI_CHATGPT_DEPLOYMENT,
...
async def close_clients():
    if current_app.config.get(CONFIG_SEARCH_CLIENT):
        await current_app.config[CONFIG_SEARCH_CLIENT].close()
    if current_app.config.get(CONFIG_BLOB_CONTAINER_CLIENT):
