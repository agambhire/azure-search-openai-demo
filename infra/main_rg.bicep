...
// param deployAzureOpenAi bool = open/AiHost == 'azure'
...
@description('Subsets')
param app_intSubnetName string = ''
param app_intSubnetPrefix string = ''
param app_intSubnetExists bool = false
param azureBastionSubnetName string = ''
param azureBastionSubnetPrefix string = ''
param azureBastionSubnetExists bool = false
param backendSubnetName string = ''
param backendSubnetPrefix string = ''
param backendSubnetExists bool = false
param vmSubnetName string = ''
param vmSubnetPrefix string = ''
param vmSubnetExists bool = false
...
module acaAuth 'core/host/container-apps-auth.bicep' = if (deploymentTarget == 'containerapps' && !empty(clientAppId)) {
  ...
  params: {
    ...
    authenticationIssuerUri: authenticationIssuerUri
    enableUnauthenticatedAccess: enableUnauthenticatedAccess
    blobContainerUri: 'https://${userStorageAccountName}.blob.${environment().suffixes.storage}/${tokenStorageContainerName}'
...
module ai 'core/ai/ai-environment.bicep' = if (useAiProject) {
  ...
  params: {
    ...
    storageAccountId: userStorage.outputs.id
...
module isolation 'network-isolation.bicep' = {
  ...
  params: {
    ...
    app_intSubnetName: !empty(app_intSubnetName) ? app_intSubnetName : 'app-int-subnet'
    appintSubnetPrefix: !empty(app_intSubnetPrefix) ? app_intSubnetPrefix : '10.0.3.0/24'
    app_intSubnetExists: app_intSubnetExists

    azureBastionSubnetName: !empty(azureBastionSubnetName) ? azureBastionSubnetName : 'AzureBastionSubnet'
    azureBastionSubnetPrefix: !empty(azureBastionSubnetPrefix) ? azureBastionSubnetPrefix : '10.0.2.0/24'
    azureBastionSubnetExists: azureBastionSubnetExists

    backendSubnetName: !empty(backendSubnetName) ? backendSubnetName : 'backend-subnet'
    backendSubnetPrefix: !empty(backendSubnetPrefix) ? backendSubnetPrefix : '10.0.1.0/24'
    backendSubnetExists: backendSubnetExists

    vmSubnetName: !empty(vmSubnetName) ? vmSubnetName : 'vm-subnet'
    vmSubnetPrefix: !empty(vmSubnetPrefix) ? vmSubnetPrefix : '10.0.4.0/24'
    vmSubnetExists: vmSubnetExists
  ...
  ? [
      {
        ...
        resourceIds: concat([userStorage.outputs.id], useUserUpload ? [userStorage.outputs.id] : [])
      }
      // {
